...
const FormAddHematologia = () => {
                ...
                .then(response => {
                    const muestrasFiltradas = response.data.filter(muestra => {
                        const muestraYaExiste = hematologia.some(hematologia => {
                            return hematologia.muestraId === muestra.id && hematologia.pacienteId
                        });
    ...
    const saveHematologia = async (e) => {
        e.preventDefault();
        try {
            await axios.post("http://localhost:5000/resultados/hematologia", {
...
export default FormAddHematologia;
