...
const FormEditHeces = () => {
                ...
                .then(response => {
                    const muestrasFiltradas = response.data.filter(muestra => {
                        const muestraYaExiste = heces.some(heces => {
                            return heces.muestraId === muestra.id && heces.pacienteId === paciente
                        });
    ...
    useEffect(() => {
        const getHecesById = async () => {
            try {
                const response = await axios.get(
                    `http://localhost:5000/resultados/heces/${id}`
                );
                setAspecto(response.data.aspecto)
                setConsistencia(response.data.consistencia);
                setColor(response.data.color);
                setOlor(response.data.olor);
                setMoco(response.data.moco);
                setSangre(response.data.sangre);
                setRestosAlimenticos(response.data.restosAlimenticios);
                setMicroscopio(response.data.microscopio);
                setOtrosElementos(response.data.otrosElementos);
    ...
    const updateHeces = async (e) => {
        e.preventDefault();
        try {
            await axios.patch(`http://localhost:5000/resultados/heces/${id}`, {
                aspecto: aspecto,
