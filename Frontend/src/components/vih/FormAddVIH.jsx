...
const FormAddVIH = () => {
            ...
            axios.get(`http://localhost:5000/muestras/pacienteId/${paciente}`)
                .then(response => {
                    const muestrasFiltradas = response.data.filter(muestra => {
                        const muestraYaExiste = vih.some(vih => {
                            return vih.muestraId === muestra.id && vih.pacienteId
                        });
    ...
    const saveVIH = async (e) => {
        e.preventDefault();
        try {
            await axios.post("http://localhost:5000/resultados/vih", {
                resultado: resultado,
                                        ...
                                        <div className="select is-multiple">
                                            <select value={muestra} name="muestra" onChange={(e) => setMuestra(e.target.value)} disabled={!paciente}>
                                                ...
                                                {Muestras.map((Muestras) => {
                                                    if (Muestras.examen === "Serología del VIH") {
                                                        return (
