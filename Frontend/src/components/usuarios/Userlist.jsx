...
const Userlist = () => {
  ...
  const datosPaginados = users.slice(inicio, fin);
  ...
  const filterOptions = () => {
    const filteredOptions = datosPaginados.filter((datosPaginados) =>
      datosPaginados.name.toLowerCase().includes(searchTerm.toLowerCase())
    );
  ...
  return (
    <div className="container">
      ...
      <div>
        ...
        {[...Array(Math.ceil(users.length / sizePagina))].map((_, i) => (
          <button key={i + 1} onClick={() => handlePageChange(i + 1)} className={i + 1 === paginaActual ? 'active button' : 'button'}>
            {i + 1}
          </button>
        ))}
        <button className="button is-link" onClick={() => handlePageChange(paginaActual + 1)} disabled={paginaActual === Math.ceil(users.length / sizePagina)}>
          Siguiente
        </button>
      </div><br />
      <div className="field">
