...
const FormEditOrina = () => {
                ...
                .then(response => {
                    const muestrasFiltradas = response.data.filter(muestra => {
                        const muestraYaExiste = orina.some(orina => {
                            return orina.muestraId === muestra.id && orina.pacienteId === paciente
                        });
    ...
    useEffect(() => {
        const getOrinaById = async () => {
            try {
                const response = await axios.get(
                    `http://localhost:5000/resultados/orina/${id}`
                );
                setAspecto(response.data.aspecto);
                setColor(response.data.color);
                setOlor(response.data.olor);
                setDensidad(response.data.densidad);
                setPh(response.data.ph);
                setProteinas(response.data.proteinas);
                setGlucosa(response.data.glucosa);
                setNitritos(response.data.nitritos);
                setC_cetonicos(response.data.c_cetonicos);
                setBilirrubina(response.data.bilirrubina);
                setUrobilinogeno(response.data.urobilinogeno);
                setHemoglobina(response.data.hemoglobina);
                setCel_epiteliales(response.data.cel_epiteliales);
                setBacterias(response.data.bacterias);
                setLeucocitos(response.data.leucocitos);
                setHematies(response.data.hematies);
                setMucina(response.data.mucina);
                setPiocitos(response.data.piocitos);
                setCilindros(response.data.cilindros);
                setCristales(response.data.cristales);
                setOtros(response.data.otros);
    ...
    const updateOrina = async (e) => {
        e.preventDefault();
        try {
            await axios.patch(`http://localhost:5000/resultados/orina/${id}`, {
                aspecto: aspecto,
                color: color,
                olor: olor,
                densidad: densidad,
